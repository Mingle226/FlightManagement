#include "pages_add.h"
#include "ui_pages_add.h"

#include <QMessageBox>

pages_add::pages_add(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::pages_add)
{
    ui->setupUi(this);
...
void pages_add::on_btn_yes_clicked()
{
    if(ui->text_id->text().compare("")==0||ui->startTime->text().compare("请选择")==0||ui->endTime->text().compare("请选择")==0)
    {
    ...
    else
    {
        ...
        QString time;
        id=ui->text_id->text();
        cityFrom=QString::number(getId(ui->cityFrom->currentText()));
        cityTo=QString::number(getId(ui->cityTo->currentText()));
        startTime=ui->startTime->text();
        endTime=ui->endTime->text();
        QStringList time1=startTime.split(":");
        QStringList time2=endTime.split(":");
        int time_int=-(time1[0].toInt()-time2[0].toInt())*60-(time1[1].toInt()-time2[1].toInt());
        time=QString::number(time_int);
        addFlight(id,cityFrom,cityTo,startTime,endTime,time);
        this->hide();
    }
}
...
void pages_add::on_btn_no_clicked()
{
    this->hide();
    ui->text_id->setText("");
    ui->cityFrom->setCurrentText("请选择");
    ui->cityTo->setCurrentText("请选择");
}
